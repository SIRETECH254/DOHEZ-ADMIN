# Edit Branch Screen Documentation
...
import { useGetBranchById, useUpdateBranch } from '../../../tanstack/useBranches';
import { useGetVendors } from '../../../tanstack/useVendors';
import { useSearchLocation } from '../../../tanstack/useLocations';
import type { IVendor, ILocationResult, WorkingHours, IBranch } from '../../../types/api.types';
```
...
#### `useGetBranchById`
- **Hook usage:** `const { data: branchData, isLoading: isBranchLoading } = useGetBranchById(id!);`
- **Purpose:** Fetches the branch details for editing.

#### `useUpdateBranch`
- **Hook usage:** `const updateBranch = useUpdateBranch();`
- **Purpose:** Mutation hook to update an existing branch.

#### `useGetVendors`
- **Hook usage:** `const { data: vendorsData, isLoading: isSearchingVendors } = useGetVendors({ search: debouncedVendorSearchQuery, limit: 5 });`
- **Purpose:** Fetches vendors for selection during branch editing.
...
#### `form`
- **Purpose:** Stores the full branch edit payload.
```tsx
...
#### `galleryFiles`, `galleryPreviews` & `existingGallery`
- **Purpose:** Manages new gallery files/previews and existing gallery images.
```tsx
...
#### `lastLoadedBranchId`
- **Purpose:** Ensures form data is synced only once when branch data loads.
```tsx
...
### `useEffect` (Data Sync)
**purpose:** Synchronize form state with branch data when it loads.

**process:**
1. Checks if `branch` data has loaded and if its ID differs from `lastLoadedBranchId`.
2. Updates `form`, `coverPreviewUrl`, `existingGallery`, and `vendorSearchQuery` states.
3. Updates `lastLoadedBranchId` to ensure the sync runs only once.

**function implementation:**
```tsx
  if (branch && branch._id !== lastLoadedBranchId) {
    setLastLoadedBranchId(branch._id);
    
    setForm({
      vendorId: typeof branch.vendorId === 'string' ? branch.vendorId : (branch.vendorId as IVendor)?._id || '',
      name: branch.name,
      email: branch.email,
      phone: branch.phone,
      location: branch.location || { address: '', coordinates: { lat: 0, lng: 0 } },
      workingHours: branch.workingHours || {},
    });
    setCoverPreviewUrl(branch.cover || null);
    setExistingGallery(branch.gallery || []);
    setVendorSearchQuery('');
  }
```

### `removeGalleryImage()`
**purpose:** Remove images from gallery, differentiating between new files and existing ones.

**process:**
1. If `isExisting` is true, removes image from `existingGallery`.
2. Otherwise, removes image from `galleryFiles` and `galleryPreviews`.
...
### `handleSubmit()`
**purpose:** Constructs `FormData`, triggers `updateBranch` mutation, and navigates.

**process:**
1. Validates required fields.
2. Creates `FormData` instance and appends form fields, files, and existing gallery JSON.
3. Calls `updateBranch.mutateAsync({ branchId: id!, data: formData })`.
...
```tsx
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      await updateBranch.mutateAsync({ branchId: id!, data: formData });
      navigate('/branches');
    } catch (err: any) {
      const errorMessage = err?.response?.data?.message || 'Failed to update branch';
      setInlineError(errorMessage);
    }
  }, [form, cover, galleryFiles, existingGallery, updateBranch, navigate, activeTab, id]);
```
...
export const branchAPI = {
  // Update branch
  updateBranch: (branchId: string, formData: FormData) => api.put(`/api/branches/${branchId}`, formData, {
...
export const useUpdateBranch = () => {
  ...
  return useMutation({
    mutationFn: async ({ branchId, data }: { branchId: string, data: FormData }) => {
...
#### Payload
`FormData` (name, email, phone, location (JSON string), workingHours (JSON string), cover (file), gallery (files), existingGallery (JSON string)).
...
## UI Structure
- Same wizard structure as `CreateBranch`.
- Shows "Edit Branch: [Name]" in the header.
- Displays existing cover and gallery images.

## Form Inputs

### `Vendor Search Input`
**Purpose**: Collects search query for selecting a vendor, pre-filled with the current vendor.
...
### `Branch Name Input`
**Purpose**: Collects the branch name, pre-filled with existing data.
...
### `Email Input`
**Purpose**: Collects the branch email address, pre-filled with existing data.
...
### `Phone Input`
**Purpose**: Collects the branch contact phone number, pre-filled with existing data.
...
**Purpose**: Collects search query for selecting a location, pre-filled with existing address.
...
**Purpose**: Collects start and end times for each day of the week, pre-filled with existing hours.
...
## Error Handling
- Handles loading state (`isBranchLoading`).
- Handles `updateBranch` mutation errors.
...
├──────────────────────────────────────────────────────────┤
│ Navigation Controls (Back/Cancel, Continue/Update)       │
└──────────────────────────────────────────────────────────┘
```
...
```
┌───────────────────────────────────────────────────────────────────────────────────────┐
│ Edit Branch: CBD Branch                                              Step 1 of 7      │
│ [ Progress Bar ]                                                                      │
│                                                                                       │
│ [ 1 ] [ 2 ] [ 3 ] [ 4 ] [ 5 ] [ 6 ] [ 7 ]                                             │
│                                                                                       │
