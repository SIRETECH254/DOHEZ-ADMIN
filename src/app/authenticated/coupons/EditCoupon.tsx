...
const EditCouponSkeleton = () => (
  <div className="p-6 max-w-5xl mx-auto space-y-6 animate-pulse">
    <div className="h-6 w-32 bg-gray-200 rounded"></div>
    <div className="h-8 w-64 bg-gray-200 rounded"></div>
    <div className="bg-white rounded-3xl h-[600px] border border-gray-100"></div>
  </div>
);
...
const EditCoupon: React.FC = () => {
  ...
  const { data: couponData, isLoading: isFetchingCoupon } = useGetCouponById(id || '');
  ...
  useEffect(() => {
    if (coupon) {
      setForm({
        ...
        vendor: (coupon.vendor as any)?._id || coupon.vendor || '',
        branch: (coupon.branch as any)?._id || coupon.branch || '',
  ...
  const handleSubmit = useCallback(async () => {
    try {
      await updateCoupon.mutateAsync({ id: id!, data: form });
      navigate('/coupons');
    } catch (err: any) {
      setInlineError(err?.response?.data?.message || 'Failed to update coupon');
    }
  }, [id, form, updateCoupon, navigate]);

  if (isFetchingCoupon) return <EditCouponSkeleton />;
          ...
          <div className="space-y-8 animate-fadeIn">
            ...
            <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 ml-2">
                ...
                <div className="space-y-1">
                    ...
                    <div className="pt-1">
                        <StatusBadge status={form.isActive} type="coupon-status" />
                    </div>
                </div>
            ...
            <div className="p-4 bg-brand-primary/5 rounded-2xl border border-brand-primary/10 flex items-start gap-3">
              ...
              <p className="text-xs text-brand-primary/80 leading-relaxed">
                Reviewing changes to "{coupon?.name}". All modifications will be saved once you click "Save Changes".
              </p>
            </div>
          </div>
        );
