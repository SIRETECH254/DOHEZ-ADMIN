...
const ProductVariantDetail: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data: variant, isLoading, isError, error } = useGetProductVariantById(id!);

  if (isLoading) return <div className="p-6 text-gray-500">Loading...</div>;
  
  if (isError) {
    const errorMessage = (error as any)?.response?.data?.message ?? 'Failed to load product variant';
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 p-6 text-center">
        <FiAlertTriangle className="text-brand-accent" size={64} />
        <h2 className="text-xl font-semibold text-gray-900">Error</h2>
        <p className="text-gray-500">{errorMessage}</p>
        <button onClick={() => navigate('/product-variants')} className="btn-primary mt-4">Back to List</button>
      </div>
    );
  }

  if (!variant) return <div className="p-6 text-gray-500 text-center">Variant not found.</div>;

  return (
    <div className="p-6 space-y-6">
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-y-3">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/product-variants')} className="p-2 hover:bg-gray-100 rounded-full text-gray-500">
            <HiOutlineArrowLeft size={24} />
          </button>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{variant.name}</h1>
            <p className="text-sm text-gray-500">ID: {variant._id}</p>
          </div>
        </div>
        <button onClick={() => navigate(`/product-variants/${id}/edit`)} className="btn-primary flex items-center gap-2">
          <HiOutlinePencil size={20} /> Edit Variant
        </button>
      </header>
