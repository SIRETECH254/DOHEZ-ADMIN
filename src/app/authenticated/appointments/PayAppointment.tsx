...
const PayAppointment: React.FC = () => {
  ...
  const payAppointmentMutation = usePayAppointmentInvoice();
  ...
  useEffect(() => {
    const initiatePayment = async () => {
      ...
      let phone = (appointment.customer as any)?.phone || user?.phone || '';
      ...
      try {
        ...
        setSocketStatus('failed');
        setSocketError(err?.response?.data?.message || 'Failed to initiate payment');
      }
    };
  ...
  }, [appointment, appointmentId, user, hasInitiated, payAppointmentMutation, startTracking]);
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className={`p-10 flex flex-col items-center text-center ${config.color} bg-opacity-5`}>
          ...
          <p className="mt-2 text-gray-600 ">
            {currentStatus === 'processing' && 'Please check the customer\'s phone for the M-Pesa PIN prompt to complete payment.'}
            {currentStatus === 'completed' && 'The payment has been successfully processed.'}
            {currentStatus === 'failed' && (socketError || 'Something went wrong with the payment.')}
            {currentStatus === 'cancelled' && 'The payment process was cancelled.'}
          </p>
          {isFallbackActive && (
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="bg-brand-primary/5 rounded-3xl p-6 border border-brand-primary/10 space-y-4">
            <div className="flex items-center justify-between border-b border-brand-primary/10 pb-4 mb-2">
              <h3 className="text-sm font-bold uppercase tracking-wider text-brand-primary">Price Breakdown</h3>
            </div>
            <div className="space-y-3">
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-600">Booking Fee (Already Paid)</span>
                <span className="font-medium text-gray-900">{formatCurrency(appointment?.bookingFeeAmount || 0)}</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-gray-600">Remaining Balance (Pay Now)</span>
                <span className="font-bold text-brand-primary">{formatCurrency(appointment?.remainingAmount || 0)}</span>
              </div>
              <div className="h-px bg-brand-primary/10 w-full" />
              <div className="flex justify-between items-center">
                <span className="font-bold text-gray-900 uppercase">Total Appointment Value</span>
        ...
        <div className="px-8 py-4 bg-gray-50 flex items-start space-x-3">
          ...
          <p className="text-sm text-gray-600 italic">
            {currentStatus === 'processing' 
              ? "Confirming this appointment requires a payment via M-Pesa STK push."
              : "An automated confirmation message has been sent."}
          </p>
        </div>
