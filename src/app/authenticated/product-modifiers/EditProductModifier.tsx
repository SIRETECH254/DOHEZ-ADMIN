...
const EditProductModifier: React.FC = () => {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data: modifierData, isLoading, isError, error } = useGetProductModifierById(id!);
  const modifier = modifierData;
  const updateProductModifier = useUpdateProductModifier();
  ...
  if (isLoading) return <div className="p-6 text-gray-500">Loading modifier data...</div>;
  if (isError) return <div className="p-6 text-red-500">Error: {(error as any)?.response?.data?.message || 'Failed to load modifier'}</div>;
  if (!modifier) return <div className="p-6 text-gray-500">Modifier not found.</div>;

  return (
    <div className="p-6">
      <button onClick={() => navigate(`/product-modifiers/${id}`)} className="flex items-center text-gray-500 mb-6 hover:text-brand-primary transition-colors">
        <MdArrowBack className="mr-2" /> Back to Details
      </button>
      
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Modifier: {modifier.name}</h1>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4 flex gap-4">
            <button type="submit" className="btn-primary flex-1" disabled={updateProductModifier.isPending}>{updateProductModifier.isPending ? 'Saving...' : 'Save Changes'}</button>
            <button type="button" onClick={() => navigate(`/product-modifiers/${id}`)} className="btn-secondary flex-1">Cancel</button>
          </div>
        </form>
      </div>
    </div>
