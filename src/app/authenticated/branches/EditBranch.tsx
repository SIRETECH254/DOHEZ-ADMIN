...
const EditBranch: React.FC = () => {
  ...
  const branch: IBranch | undefined = branchData?.branch;
  ...
  // Synchronize form with branch data when it loads
  ...
  const selectedVendor = vendors.find((v: IVendor) => v._id === form.vendorId) || 
                         (branch && typeof branch.vendorId !== 'string' && (branch.vendorId as IVendor)?._id === form.vendorId ? (branch.vendorId as IVendor) : null);
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // We send existing gallery URLs/IDs so the backend knows what to keep
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {existingGallery.map((item, idx) => (
                    <div key={`existing-${idx}`} className="relative aspect-square rounded-2xl overflow-hidden group border border-gray-100">
                      <img src={item.url} alt={`Gallery ${idx}`} className="h-full w-full object-cover" />
                      <button 
                        type="button"
        ...
        return (
          <div className="space-y-4 animate-fadeIn">
            ...
            <div className="space-y-1">
              ...
              <input type="text" value={form.location.address} readOnly className="input bg-gray-50 text-gray-500" />
            </div>
          </div>
        );
      case 'hours':
        return (
          <div className="space-y-4 animate-fadeIn max-h-[60vh] overflow-y-auto pr-2">
            ...
            {DAYS.map((day) => {
              const dayHours = (form.workingHours as any)[day] || { start: '08:00', end: '18:00' };
              const isClosed = dayHours.start === '00:00' && dayHours.end === '00:00';
                  ...
                  <div className="ml-4">
                    ...
                    <button 
                      ...
                      onClick={() => setForm({
                        ...
                        workingHours: {
                          ...form.workingHours,
                          [day]: isClosed ? { start: '08:00', end: '18:00' } : { start: '00:00', end: '00:00' }
                        }
                      })}
                      className={`h-6 w-10 rounded-full transition-colors relative ${isClosed ? 'bg-red-400' : 'bg-green-400'}`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isClosed ? 'left-5' : 'left-1'}`} />
                    </button>
                  </div>
                </div>
              );
          ...
          <div className="space-y-8 animate-fadeIn">
              ...
              <div className="ml-2">
                {existingGallery.length === 0 && galleryPreviews.length === 0 ? (
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-6 gap-3">
                    {[...existingGallery, ...galleryPreviews.map(url => ({url}))].map((item, idx) => (
            ...
            <div className="mt-8 p-6 bg-brand-primary/5 rounded-3xl border border-brand-primary/10 flex items-start gap-4">
              ...
              <p className="text-sm text-brand-primary/80 leading-relaxed">
                Review all changes before updating. By clicking "Update Branch", you confirm these details are correct.
              </p>
            </div>
          </div>
        );
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Edit Branch: {branch?.name}</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="pt-6 border-t border-gray-100 flex items-center justify-between gap-4">
              ...
              {activeTab === 'summary' ? (
                ...
                >
                  {updateBranch.isPending ? 'Processing...' : 'Update Branch'}
                </button>
              ) : (
                <button
                  key="continue-btn"
