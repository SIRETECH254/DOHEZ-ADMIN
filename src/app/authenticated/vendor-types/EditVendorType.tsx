...
const EditVendorType: React.FC = () => {
  ...
  if (isLoading) return <div className="p-6 text-gray-500">Loading vendor type data...</div>;
  if (isError) return <div className="p-6 text-red-500">Error: {(error as any)?.response?.data?.message || 'Failed to load vendor type'}</div>;
  if (!vendorType) return <div className="p-6 text-gray-500">Vendor type not found.</div>;

  return (
    <div className="p-6">
      <button onClick={() => navigate(`/vendor-types/${id}`)} className="flex items-center text-gray-500 mb-6 hover:text-brand-primary transition-colors">
        <MdArrowBack className="mr-2" /> Back to Vendor Type Details
      </button>
      
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Edit Vendor Type: {vendorType.name}</h1>
      </div>
      ...
      <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 ">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-4 flex gap-4">
            <button type="submit" className="btn-primary flex-1" disabled={updateVendorType.isPending}>{updateVendorType.isPending ? 'Saving...' : 'Save Changes'}</button>
            <button type="button" onClick={() => navigate(`/vendor-types/${id}`)} className="btn-secondary flex-1">Cancel</button>
          </div>
        </form>
      </div>
    </div>
