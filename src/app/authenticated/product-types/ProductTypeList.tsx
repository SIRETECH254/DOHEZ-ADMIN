...
const ProductTypeList: React.FC = () => {
  ...
  const { data, isLoading, isError, error } = useGetProductTypes(params);
  const deleteProductType = useDeleteProductType();

  const productTypes = data?.productTypes || [];
  const pagination = data?.pagination ?? { currentPage: 1, totalPages: 1, totalProductTypes: 0 };
    ...
    <div className="p-6 space-y-6">
      <header>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-gray-500">Manage types of products</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-4">
          ...
          <Link to="/product-types/new" className="btn-primary flex items-center gap-2 w-full sm:w-auto">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
          <div>
            <p className="text-sm text-gray-500">Showing {pagination.totalProductTypes} product types</p>
          </div>
      ...
      <div className="table-container">
        <table className="table">
          ...
          <tbody className="table-body">
            ...
            {!isLoading && !isError && productTypes.map((type: IProductType) => (
              <tr key={type._id} className="table-row">
                <td className="table-cell font-medium text-gray-900 whitespace-nowrap">
                  <div className="flex items-center gap-3">
                    {type.icon ? (
                      <img src={type.icon} alt={type.name} className="h-10 w-10 rounded-full object-cover" />
                    ) : (
                ...
                <td className="table-cell whitespace-nowrap">{(type.service as IService)?.name || 'N/A'}</td>
                <td className="table-cell">{type.order}</td>
                <td className="table-cell text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button onClick={() => navigate(`/product-types/${type._id}`)} className="text-green-600"><HiOutlineEye size={20} /></button>
                    <button onClick={() => navigate(`/product-types/${type._id}/edit`)} className="text-blue-600"><HiOutlinePencil size={20} /></button>
                    <button onClick={() => handleDeleteClick(type._id, type.name)} className="text-red-600"><HiOutlineTrash size={20} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {!isLoading && !isError && pagination.totalPages > 1 && (
        <div className="mt-4">
          <Pagination
            ...
            totalItems={pagination.totalProductTypes}
            currentPageCount={productTypes.length}
