...
const EditEvent: React.FC = () => {
  ...
  const updateEvent = useUpdateProduct();
  ...
  useEffect(() => {
    if (product) {
      ...
      setForm({
        ...
        service: typeof product.service === 'string' ? product.service : (product.service as IService)?._id || '',
        category: typeof product.category === 'string' ? product.category : (product.category as IProductCategory)?._id || '',
        vendor: typeof product.vendor === 'string' ? product.vendor : (product.vendor as IVendor)?._id || '',
        branch: typeof product.branch === 'string' ? product.branch : (product.branch as IBranch)?._id || '',
  ...
  // Search Hooks (Categories, Services, Vendors, Branches, Locations)
  const [categorySearch, setCategorySearch] = useState('');
  const [debouncedCategorySearch, setDebouncedCategorySearch] = useState('');
  const { data: categoriesData } = useGetProductCategories({ search: debouncedCategorySearch });
  ...
  const { data: servicesData } = useGetServices({ search: debouncedServiceSearch });
  ...
  const { data: vendorsData } = useGetVendors({ search: debouncedVendorSearch });
  ...
  const allVariants = (variantsData as any)?.variants || [];
  ...
  const toggleVariantOption = (variantId: string, optionId: string) => {
    setForm(prev => {
      const isSelected = prev.selectedVariantOptions.some(so => so.variantId === variantId && so.optionId === optionId);
      let nextSelectedOptions = isSelected 
        ? prev.selectedVariantOptions.filter(so => !(so.variantId === variantId && so.optionId === optionId))
        : [...prev.selectedVariantOptions, { variantId, optionId }];

      const hasOptionsSelected = nextSelectedOptions.some(so => so.variantId === variantId);
      let nextVariants = hasOptionsSelected && !prev.variants.includes(variantId) 
        ? [...prev.variants, variantId] 
        : (!hasOptionsSelected && prev.variants.includes(variantId) ? prev.variants.filter(id => id !== variantId) : prev.variants);
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <div className="absolute right-3 top-1/2 -translate-y-1/2">
                    {false ? <div className="animate-spin h-4 w-4 border-2 border-brand-primary border-t-transparent rounded-full" /> : <HiOutlineSearch className="text-gray-400" />}
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                {existingImages.map((img, idx) => (
                    <div key={`exist-${idx}`} className="relative aspect-square rounded-2xl overflow-hidden border border-gray-100 shadow-sm group">
                        <img src={img.url} className="w-full h-full object-cover" alt="Existing" />
                        <button onClick={() => removeImage(idx, true)} className="absolute top-1 right-1 h-6 w-6 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <HiOutlineTrash size={14} />
                        </button>
                    </div>
                ))}
                {previewUrls.map((url, idx) => (
                    <div key={`new-${idx}`} className="relative aspect-square rounded-2xl overflow-hidden border border-gray-100 shadow-sm group">
                        <img src={url} className="w-full h-full object-cover" alt="Preview" />
                        <button onClick={() => removeImage(idx, false)} className="absolute top-1 right-1 h-6 w-6 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <HiOutlineTrash size={14} />
                        </button>
                    </div>
                ))}
              ...
              <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100 space-y-4">
                ...
                <div className="space-y-2">
                  {form.variants.length > 0 ? form.variants.map(id => {
                    const v = allVariants.find((v: IVariant) => v._id === id);
  ...
  if (isProductLoading) return <div className="p-6 h-[60vh] flex items-center justify-center"><div className="animate-spin h-12 w-12 border-4 border-brand-primary border-t-transparent rounded-full" /></div>;
        ...
        <div className="p-5">
          <form className="space-y-8">
            ...
            <div className="pt-6 border-t border-gray-100 flex items-center justify-between gap-4">
              ...
              {activeTab === 'summary' ? (
                <button type="button" onClick={handleSubmit} className="btn-primary px-12" disabled={updateEvent.isPending}>{updateEvent.isPending ? 'Saving...' : 'Save Changes'}</button>
              ) : (
